	...
	,
   	...
   	OnCheckDeliver = function(TASKINTERFACE_POINTER)
		...
		local iOccupation = TaskCApi.GetPlayerOccupation(TASKINTERFACE_POINTER)--获取玩家职业
	   	...
	   	if iPlayerLev < 200 then --等级需要高于75级（修改英雄套获得难度：将任务第一步接受等级提高，不再发放）
	   	    return TASK_PREREQU_FAIL_LUA_TASK
	   	end
...
TaskList[12760] = --武器师日常
{
	...
	AwardPreview =
	{
		...
		SideOccupExp = function(TASKINTERFACE_POINTER)
			local soe = {}
			local tabSideOccupExp = {20,50,100,200,380,728,1000,1500,1500,1500}							--存放副职业经验奖励
	...
	,
   	...
   	OnCheckDeliver = function(TASKINTERFACE_POINTER)
		...
		local iSideOccupIdhighlevel = 8 --当前满级武器师为8级	--2013.09.07改
...
TaskList[12763] = --防具师日常
{
	...
	AwardPreview =
	{
		...
		SideOccupExp = function(TASKINTERFACE_POINTER)
			local soe = {}
			local tabSideOccupExp = {60,92,162,310,578,1098,1500,1500,1500,1500}						--存放副职业经验奖励
	...
	,
   	...
   	OnCheckDeliver = function(TASKINTERFACE_POINTER)
		...
		local iSideOccupIdhighlevel = 8 --当前满级防具师为8级	--2013.09,07改
		...
		if itemIDSubmitted and itemNumberSubmitted then
			...
			if iTaskID then --标志任务id不为空，属于需要交纳内容
				...
				elseif TabFlag[iTaskID] == 0 then
					...
					--拆解物品id，存入随机数；key1*200+key2 =itemIDSubmitted
   	...
   	OnAward = function(TASKINTERFACE_POINTER, bSuccess,iParam,bRetrieve,ucRandomNum1,ucRandomNum2)
		...
		local AwardExpAdjust = TaskExpAdjust[PlayerLevel] 											--玩家等级的修正经验
   		...
   		if bSuccess then
			...
			local iTaskId =TaskList[14094].Tabtaskmethod[ucRandomNum1*200+ucRandomNum2]--需要发放的标志任务
...
TaskList[14262] = --2012春节活动种符文
{
	--任务完成方式

	TabRuneExchage =
	{
		--[职业id] = {种子符文，收获红符文，收获的黄符文，收获的蓝符文}
	...
	AwardPreview =
	{
		...
		Items = function(TASKINTERFACE_POINTER)
			local items = {}
			items = {
					Item1 = {ID = 41385, NUM = 1},--显示的奖励为一个不会出现的物品41385，仅用于显示奖励用
					}
	...
	,
   	...
   	OnCheckDeliver = function(TASKINTERFACE_POINTER)
	    ...
	    local iPlayerLev = TaskCApi.GetPlayerLevel(TASKINTERFACE_POINTER)
	   	...
	   	if iPlayerLev < 45 then --等级需要高于45级
	   	    return TASK_PREREQU_FAIL_LUA_TASK
	   	end
	...
	,
   	...
   	OnMethod = function(TASKINTERFACE_POINTER)
		...
		local Method = {}
		Method.SubmitItem = true--提交物品的完成方式
		Method.iTimeLimit = 3628800 --1小时没完成则任务失败
   	...
   	OnCheckFinish = function(TASKINTERFACE_POINTER)
		...
		local iTaskId = 14262 --本任务id
		local iOccupationId = TaskCApi.GetPlayerOccupation(TASKINTERFACE_POINTER) --获取玩家职业id
		...
		local iRuneOccNum = 39 --每个职业可收取的符文个数
		local bFlag = TaskCApi.GetDataFromActiveTaskList(TASKINTERFACE_POINTER,iTaskId, 1)--1号随机数用于存储是否交纳种子符文，如果为0则为未交纳

		local itemIDSubmitted,itemNumberSubmitted = TaskCApi.GetSubmitItemInfo(TASKINTERFACE_POINTER,iTaskId)	--提交物品的信息：id，数量

		if itemIDSubmitted and itemNumberSubmitted then

			if bFlag == 0 then		--玩家并未提交种子符文

				for i = 1 , iRuneOccNum do
					if itemIDSubmitted == TaskList[iTaskId].TabRuneExchage[iOccupationId][i][1] then--如果提交的内容是本职业原始符文
						if itemNumberSubmitted == 1 then --数量正确则收取并提交
							...
							TaskCApi.NotifySubmitItemErr(TASKINTERFACE_POINTER, iTaskId, 29, 20001)--数量不对
						end
						break
					end

					if i == iRuneOccNum then--提交的物品错误
				...
				if itemIDSubmitted == 41302 then	--玩家提交的是种植凭证
					if bflagred or bflagyel or bflagblu then --已经选择颜色，有一项为真
						if itemNumberSubmitted == 1 then --提交数量正确收走物品发奖
							TaskCApi.TakeAwaySubmitItem(TASKINTERFACE_POINTER,iTaskId, 1)
							iRet = TASK_SUCC_FINISH
						else--数量不对
							TaskCApi.NotifySubmitItemErr(TASKINTERFACE_POINTER, iTaskId, 29, 20001)--数量不对
						end
					else--如果玩家没有选择颜色，则不收取并提示一个自定义错误信息
						TaskCApi.NotifySubmitItemErr(TASKINTERFACE_POINTER, iTaskId, 28, 6)--打印一句错误提示，告知玩家未选择颜色
					end
				else--提交的物品错误
					TaskCApi.NotifySubmitItemErr(TASKINTERFACE_POINTER, iTaskId, 28, 20000)
				end
   	...
   	OnAward = function(TASKINTERFACE_POINTER, bSuccess,iParam,bRetrieve,ucRandomNum1)
		...
		local bflagcol = 2 --默认选2，玩家选择的颜色，2为红色，3为黄色，4为蓝色
		local iAwardRuneId --要发放的符文id
...

{
	...
	AwardPreview =
	{
		...
		Exp = function(TASKINTERFACE_POINTER)
			...
			local FinishTime = TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER, iTaskId) + 1--环任务完成次数（包括本次）
	...
	,
   	...
   	--任务发放检查，客户端在NPC显示、玩家确认接受时调用，服务器在响应发放时调用
   	OnCheckDeliver = function(TASKINTERFACE_POINTER)
		local iItemId = 23911 --存储前提所需公会事物函id
		local iOldTaskId = 8547 --旧的建设任务id 8547
		local iTaskId = 14379 --本任务id
		...
		--每天第一次做任务时检查是否有牌子id=23911
		local FinishTime = TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER, iTaskId)
		local iOldFinishTime = TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER, iOldTaskId) --旧基地建设任务完成次数
   	...
   	OnDeliver = function(TASKINTERFACE_POINTER)
		...
		local iTaskId = 14379   --本任务id 14379
		local iTaskId2 = 14380  --新的基地非建设任务14380
		local iFlagTaskID = 14578 --用于显示任务追踪的任务，标示辅助任务是否完成

		local iOldTaskId = 8547 --旧的建设任务id 8547
		local iItemId = 23911   --工会事物函 23911

		local iTaskType = 1     --任务难度类型，1为简单（默认），2为难

		local iPlayerLev = TaskCApi.GetPlayerLevel(TASKINTERFACE_POINTER)	--玩家等级

		--每轮（4次）基地任务首次时收走一个基地事务函
		local iFinishTime = TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER, iTaskId) --本任务当天已完成次数
		local iOldFinishTime = TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER, iOldTaskId) --就基地建设任务完成次数
		...
		if iFinishTime == 0 and iOldFinishTime == 0 then
			TaskCApi.TakeAwayCommonItem(TASKINTERFACE_POINTER, iItemId, 1) --收走一个工会事物函
		end

		--如果旧的建设任务次数非0，将旧的任务完成次数继承到新任务上
		if iOldFinishTime > 0 then
			TaskCApi. ModifyTaskFinishCnt (TASKINTERFACE_POINTER, iTaskId, iOldFinishTime)  --修改新建设任务14379的已完成次数（继承旧任务的次数）
		end

		--清空标志任务次数，用于完成任务判断
		if iFlagFinishiTime >0 then
			TaskCApi.ClearTaskCompleteTimesInFinishTimeTaskList(TASKINTERFACE_POINTER,iFlagTaskID)   --修改标志任务次数
		end

		--清空旧建设任务的次数，防止下次接受时重复继承
		TaskCApi.ClearTaskCompleteTimesInFinishTimeTaskList(TASKINTERFACE_POINTER,iOldTaskId)

		math.randomseed(os.time())
		local iRandomNum = math.random(1, 10)

		--2012.12.28任务修改，公会基地任务不再发放复杂任务
		if iRandomNum <= 0 then
		--20%发放复杂任务
			iTaskType = 2
		end
		...
		local iRandomTaskNum = math.random(1, #TaskList[iTaskId].TabSubTask[iTaskType][iPlayerLevIndex])--在已经确认的等级段内随机范围
		local iDeliverTaskID = TaskList[iTaskId].TabSubTask[iTaskType][iPlayerLevIndex][iRandomTaskNum]--将要发放的任务库id

		--由于与非建设任务的库任务共享，所以要避免发放重复任务,对iDeliverTaskID做特殊处理
		...
		TaskCApi.SetDataToActiveTaskList(TASKINTERFACE_POINTER,iTaskId, 3, iTaskType) --任务难度系数存入3号随机数
	...
	,
   	...
   	OnMethod = function(TASKINTERFACE_POINTER)
		...
		Method.CompleteTask = {TaskID = iFlagTaskID, Num = 1}	--完成特定任务1次
   	...
   	OnCheckAward = function(TASKINTERFACE_POINTER, bSuccess)
		...
		local iTaskId = 14380   --本任务id 14380
		local FinishTime = TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER, iTaskId) + 1--环任务完成次数（包括本次）
   	...
   	OnAward = function(TASKINTERFACE_POINTER, bSuccess,iParam,bRetrieve,ucRandomNum1,ucRandomNum2,ucRandomNum3)
		...
		local iLucResourcetype = 9 --幸运资源值id
		...
		local iPersonalActive = 1		--活跃度奖励数值

		local iExpMuti = 400	--特殊奖励经验系数
		local iMonMuti = 150	--特殊奖励魂币系数
		...
		if bSuccess then
			...
			if FinishTime > 0 then
				...
				if FinishTime == 4 then --4环结束时清空任务完成次数
					...
					TaskCApi.DeliverNewTask(TASKINTERFACE_POINTER, 16106)--2013.1.4流光优化相关
					...
					--取14381完成次数判断是否改发放额外奖励,如果没完成过则发放任务，用于作为领奖任务的前提。
			...
			--随机到难任务的额外奖励
			if ucRandomNum3 == 1 then--2013.1.4修改，简单任务触发宝箱

				local iGuildCtrlIdlev1 = 103522 --公会_基地任务1等奖 控制器id
				local iGuildCtrlIdlev2 = 103523 --公会_基地任务2等奖 控制器id
				local iGuildCtrlIdlev3 = 103524 --公会_基地任务3等奖 控制器id

				local iBroadTaskid1 = 14584 --喊话0-1
				local iBroadTaskid2 = 14585 --喊话0-2
				local iBroadTaskid3 = 14586 --喊话1-2
				local iBroadTaskid4 = 14587 --喊话2-3

				local iLucResourceVal = TaskCApi. GetFactionResourceVal(TASKINTERFACE_POINTER, iLucResourcetype)

				math.randomseed(os.time())
				local iAwardnum = math.random(1, 50000)   --公会宝箱额外开启奖励随机数

				if iLucResourceVal == 0 then--目前未开启任何奖励
					--1-200 0-1级，201-210 0-2级；
					if iAwardnum <= 200 then
						TaskCApi.DeliverExperience(TASKINTERFACE_POINTER, iExpMuti*iExpAdjust) 	--额外发放经验
						TaskCApi.DeliverBindMoney(TASKINTERFACE_POINTER, iMonMuti*iMonAdjust)	--额外发放魂币

						--发放辅助广播任务进行工会广播
						TaskCApi.DeliverNewTask(TASKINTERFACE_POINTER, iBroadTaskid1)

						TaskCApi. MonsterCtrl (TASKINTERFACE_POINTER ,iGuildCtrlIdlev1, true) --激活控制器1级的控制器

						TaskCApi.DeliverFactionResourceVal(TASKINTERFACE_POINTER,iLucResourcetype, 1) --设置幸运资源值(0+1=1)，标示开启宝箱的等级
					elseif iAwardnum <= 210 and iAwardnum > 200 then
						...
						TaskCApi. MonsterCtrl (TASKINTERFACE_POINTER ,iGuildCtrlIdlev2, true) 	 --激活控制器2级的控制器

						TaskCApi.DeliverFactionResourceVal(TASKINTERFACE_POINTER,iLucResourcetype, 2) --设置幸运资源值(0+2=2)，标示开启宝箱的等级
					end

				elseif iLucResourceVal == 1 then --目前已经开启1等奖
					--0.5% 1级升级到2级
					if iAwardnum <= 50 then
						...
						TaskCApi. MonsterCtrl (TASKINTERFACE_POINTER ,iGuildCtrlIdlev2, true) 	 --激活控制器2级的控制器
						TaskCApi. MonsterCtrl (TASKINTERFACE_POINTER ,iGuildCtrlIdlev1, false)  --关闭控制器1级的控制器

						TaskCApi.DeliverFactionResourceVal(TASKINTERFACE_POINTER,iLucResourcetype, 1) --设置幸运资源值(1+1=2)，标示开启宝箱的等级
					end

				elseif iLucResourceVal == 2 then --目前已经开启2等奖
					--0.15% 2级升级到3级
					if iAwardnum <= 15 then
						...
						TaskCApi. MonsterCtrl (TASKINTERFACE_POINTER ,iGuildCtrlIdlev3, true) 	 --激活控制器3级的控制器
						TaskCApi. MonsterCtrl (TASKINTERFACE_POINTER ,iGuildCtrlIdlev2, false)  --关闭控制器2级的控制器

						TaskCApi.DeliverFactionResourceVal(TASKINTERFACE_POINTER,iLucResourcetype, 1) --设置幸运资源值(2+1=3)，标示开启宝箱的等级
					end
...

{
	...
	,
  	...
  	Property =
  	{
		...
		bClearAsGiveup = false, --放弃任务发放失败奖励
	...
	AwardPreview =
	{
		...
		FactionAward = function(TASKINTERFACE_POINTER)
			...
			local iPlayerLevel = TaskCApi.GetPlayerLevel(TASKINTERFACE_POINTER)
			local iAwardPlayerLevel --20级档，发放奖励的等级段，分别为20，40，60
			...
			factionaward.credit = ExploitTable[iAwardPlayerLevel][FinishTime]									--发放工会功勋度
			factionaward.contribution = ContributionTable[iAwardPlayerLevel][FinishTime] 		--发放贡献度
			factionaward.money = GuildMonTable[iAwardPlayerLevel][FinishTime]					--发放工会资金
			factionaward.vitality = 1															--公会活跃度

			factionaward.magic	 = TaskList[iTaskId].TabResourceAward[1][iAwardPlayerLevel][FinishTime]		--发放2#工会资源
			factionaward.economy = TaskList[iTaskId].TabResourceAward[2][iAwardPlayerLevel][FinishTime] 	--发放3#工会资源
			factionaward.culture = TaskList[iTaskId].TabResourceAward[3][iAwardPlayerLevel][FinishTime]		--发放4#工会资源
			factionaward.belief  = TaskList[iTaskId].TabResourceAward[4][iAwardPlayerLevel][FinishTime]		--发放5#工会资源
	...
	,
   	...
   	OnCheckDeliver = function(TASKINTERFACE_POINTER)
		...
		local TabOldTaskId = {8548,8549,8550,8551,10368,10369,10370,10371,10444,10445,} --旧的非建设任务id表
		local iTaskId = 14380 --本任务id

		local iFlagTaskID = 14579 --用于显示任务追踪的任务，标示辅助任务是否完成
		...
		local iOldFinishTime = 0 --旧基地建设任务完成次数
   	...
   	OnDeliver = function(TASKINTERFACE_POINTER)
		...
		--如果旧的非建设任务次数非0，将旧的任务完成次数继承到新任务上
		if iOldFinishTime > 0 then
			TaskCApi. ModifyTaskFinishCnt (TASKINTERFACE_POINTER, iTaskId, iOldFinishTime)  --修改新建设任务14380的已完成次数（继承旧任务的次数）
		end

		--清空旧任务的次数，防止下次接受时重复继承
		...
		--2012.12.28任务修改，公会基地任务不再发放复杂任务，概率由20%改为0
		if iRandomNum <= 0 then
		--20%发放复杂任务
			iTaskType = 2
		end
		...
		--由于与建设任务的库任务共享，所以要避免发放重复任务,对iDeliverTaskID做特殊处理
		if TaskCApi.HasTaskInActiveTaskList(TASKINTERFACE_POINTER,iDeliverTaskID) then
			if iRandomTaskNum < #TaskList[iTaskId].TabSubTask[iTaskType][iPlayerLevIndex] then
				iDeliverTaskID = TaskList[iTaskId].TabSubTask[iTaskType][iPlayerLevIndex][iRandomTaskNum +1]
			else
	...
	,
   	...
   	OnAward = function(TASKINTERFACE_POINTER, bSuccess,iParam,bRetrieve,ucRandomNum1,ucRandomNum2,ucRandomNum3)
		...
		local iExpMuti = 400 --特殊触发的奖励，经验系数
		local iMonMuti = 150 --特殊触发的奖励，魂币系数

		local iPersonalActive = 1		--活跃度奖励数值

		local iTaskId = 14380   --本任务id 14380
		local iLucResourcetype = 9  --幸运值资源值id
		...
		if bSuccess then
			...
			if FinishTime > 0 then
				TaskCApi.DeliverExperience(TASKINTERFACE_POINTER, ExpTable[FinishTime]*iExpAdjust) 	--发放经验
				TaskCApi.DeliverBindMoney(TASKINTERFACE_POINTER, MonTable[FinishTime]*iMonAdjust)	--发放魂币

				TaskCApi.DeliverFactionCredit(TASKINTERFACE_POINTER, ExploitTable[iAwardPlayerLevel][FinishTime]) 	--发放公会功勋度
				TaskCApi.DeliverFactionVitality(TASKINTERFACE_POINTER, iPersonalActive) 							--发放公会活跃度

				TaskCApi.DeliverFactionContribution(TASKINTERFACE_POINTER, ContributionTable[iAwardPlayerLevel][FinishTime]) --发放贡献度

				TaskCApi.DeliverFactionMoney(TASKINTERFACE_POINTER, GuildMonTable[iAwardPlayerLevel][FinishTime])	--发放工会资金

				TaskCApi.DeliverFactionResourceVal(TASKINTERFACE_POINTER, 1 ,TaskList[14380].TabResourceAward[1][iAwardPlayerLevel][FinishTime]) --发放2#工会资源
				TaskCApi.DeliverFactionResourceVal(TASKINTERFACE_POINTER, 2 ,TaskList[14380].TabResourceAward[2][iAwardPlayerLevel][FinishTime]) --发放3#工会资源
				TaskCApi.DeliverFactionResourceVal(TASKINTERFACE_POINTER, 3 ,TaskList[14380].TabResourceAward[2][iAwardPlayerLevel][FinishTime]) --发放4#工会资源
				TaskCApi.DeliverFactionResourceVal(TASKINTERFACE_POINTER, 4 ,TaskList[14380].TabResourceAward[2][iAwardPlayerLevel][FinishTime]) --发放5#工会资源

				if FinishTime == 4 then --4环结束时清空任务完成次数
					TaskCApi.DeliverNewTask(TASKINTERFACE_POINTER, 16106)--发放流光优化任务16106，2013.1.4
			...
			if ucRandomNum3 == 1 then
				...
				math.randomseed(os.time())
				local iAwardnum = math.random(1, 50000)   --公会宝箱额外开启奖励随机数,原随机数为10000，难任务出现概率20%，现将概率降低为原来的1/5
...
TaskList[15201] = --秋收节：祭奠英灵
{
	...
	,
   	...
   	OnDeliver = function(TASKINTERFACE_POINTER)
		...
		local timePackage = os.date("*t", os.time())
		local dayInYear = timePackage.yday
		local iRandomMethod =  ( timePackage.yday + RoleCreateTime )% 7 + 1 --玩家当天随到的任务种类(一共7种物品)
		...
		if TaskCApi.GetItemNumInPacket(TASKINTERFACE_POINTER, 4493, true) > 0 then
   		...
   		--发道具或奖励
		TaskCApi.DeliverCommonItem(TASKINTERFACE_POINTER, TaskList[iTaskID].TabItemID[iRandomMethod], 1, true)     --发 任务物品
	...
	,
   	...
   	OnMethod = function(TASKINTERFACE_POINTER)
		...
		local iTaskId = 15201   --本任务id

		local key = TaskCApi.GetDataFromActiveTaskList(TASKINTERFACE_POINTER, iTaskId, 1)

   		local Method = {}

		Method.CompleteTask = {TaskID = TaskList[iTaskId].TabComTaskID[key], Num = 1}	--完成特定任务1次
   	...
   	OnAward = function(TASKINTERFACE_POINTER, bSuccess,iParam,bRetrieve,ucRandomNum1)
		...
		local AwardMulti = 200																		--存放角色经验奖励系数
		local AwardExpAdjust = TaskExpAdjust[PlayerLevel] 											--玩家等级的修正经验
		local iBindMoneynum = 200000
		local iTitleID = 3128
		local iTaskId = 15201
		local iAwardTaskId = 15709																	--浮云任务奖励id

   		if bSuccess then

			TaskCApi.DeliverBindMoney(TASKINTERFACE_POINTER,iBindMoneynum)							--发放绑定魂币
			TaskCApi.DeliverExperience(TASKINTERFACE_POINTER,AwardMulti*AwardExpAdjust)				--发放经验
			...
			if PlayerLevel >=65 then
				...
				AwardNum = math.random(1,1000)
				if AwardNum <= 5 then
					TaskCApi.DeliverNewTask(TASKINTERFACE_POINTER,iAwardTaskId) --浮云任务奖励，信仰远能
				end
			end

			local iTaskId2 = 15389	--需要发放的标志任务

			if TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER,iTaskId2) < 6 then
				TaskCApi.DeliverNewTask(TASKINTERFACE_POINTER,iTaskId2)--如果本次不是第7次，发放flag任务以计数
			elseif TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER,iTaskId2) == 6 then
				TaskCApi.DeliverNewTask(TASKINTERFACE_POINTER,iTaskId2)--如果本次是第7次，发放flag任务以计数
			end
...
TaskList[15294] = --秋收节种植收获任务
{
	...
	,
   	...
   	OnDeliver = function(TASKINTERFACE_POINTER)
		local iTaskID = 15294
		local TabItemID ={43211, 43212,} --几类任务的物品
		...
		if TaskCApi.GetItemNumInPacket(TASKINTERFACE_POINTER, 4493, true) > 0 then
   		...
   		--发道具或奖励
		TaskCApi.DeliverCommonItem(TASKINTERFACE_POINTER, TabItemID[iRandomMethod], 1, true)     --发 任务物品

		--如果是收获任务，则清除完成次数后，发放辅助任务15296
		if iRandomMethod == 2 then
	...
	,
   	...
   	OnMethod = function(TASKINTERFACE_POINTER)
   		...
   		local tabFlagTaskID = { 15295, 15296,}
		local iTaskId = 15294   --本任务id

		local key = TaskCApi.GetDataFromActiveTaskList(TASKINTERFACE_POINTER, iTaskId, 1)

   		local Method = {}

		Method.CompleteTask = {TaskID = tabFlagTaskID[key], Num = 1}	--完成特定任务1次
   	...
   	OnAward = function(TASKINTERFACE_POINTER, bSuccess,iParam,bRetrieve,ucRandomNum1)
		...
		local iTitleID1 = 3126		--种植称号
		local iTitleID2 = 3127		--收获称号

		local iAwardItemID1 = 41463		--素颜卷
		local iAwardItemID2 = 41464		--理发卷
   		...
   		if bSuccess then
			...
			if ucRandomNum1 == 1 then
				...
				--失败或者放弃的话需要清理任务15295次数
				TaskCApi.ClearTaskCompleteTimesInFinishTimeTaskList(TASKINTERFACE_POINTER,15295)

			elseif ucRandomNum1 == 2 then
				--失败或者放弃的话需要清理任务15296次数
				TaskCApi.ClearTaskCompleteTimesInFinishTimeTaskList(TASKINTERFACE_POINTER,15296)
				--失败或者放弃的话需要清理任务中止任务15296
...
TaskList[15295] = --秋收节种植:种子任务
{
	--任务自定义表
	TabImpValue =
	{
		--索引项为可种植土地区域，二层索引为土地编号，内容为（变量id，控制器id）
	...
	,
   	...
   	--任务发放检查，客户端在NPC显示、玩家确认接受时调用，服务器在响应发放时调用
   	OnCheckDeliver = function(TASKINTERFACE_POINTER)
		local iMainTaskID = 15294	--种植收获主任务
		local iTaskID = 15295		--本任务id

		if not TaskCApi.HasTaskInActiveTaskList(TASKINTERFACE_POINTER,iMainTaskID) then --关联任务15294种植收获主要任务
   			return TASK_PREREQU_FAIL_RELATION_TASK
   		end

		local iMapId, x, y, z = TaskCApi.GetPlayerCurSite(TASKINTERFACE_POINTER) --取玩家坐标
		local index
		...
		for i = 1 , #TaskList[iTaskID].TabLocation do
			...
			--如果循环检测到最后一项，玩家所在地图号仍然不符合坐标需求范围，则无法接受任务
			if i == #TaskList[iTaskID].TabLocation then
				return TASK_PREREQU_FAIL_LUA_TASK
			end
		end

		--当玩家坐标符合要求，检测当前场地是否有空地
   	...
   	OnDeliver = function(TASKINTERFACE_POINTER)
		...
		for i = 1, #TaskList[iTaskID].TabImpValue[index] do
			if TaskCApi.GetGlobalDataEx(TASKINTERFACE_POINTER ,TaskList[iTaskID].TabImpValue[index][i][1]) == 0 then
				TaskCApi.ModifyGlobalDataEx(TASKINTERFACE_POINTER ,TaskList[iTaskID].TabImpValue[index][i][1],1)--修改全局变量
				TaskCApi. MonsterCtrl (TASKINTERFACE_POINTER ,TaskList[iTaskID].TabImpValue[index][i][2], true)--开启控制器
				break
			end
...
TaskList[15963] = --冬赐节礼盒领取记录清空用隐藏任务
{
  	Property =
  	{
  		bCanRedo = true,		--成功可重复完成
   	...
   	OnAward = function(TASKINTERFACE_POINTER, bSuccess)
   		if bSuccess then
			...
			for i, v in pairs(TabTaskID) do
				TaskCApi.ClearFinishRecordsOnly(TASKINTERFACE_POINTER, v)--清空任务结果
			end
...
TaskList[16353] = --公会功勋抽奖，yl
{
  	Property =
  	{
  		bCanRedo = true, --是否成功后可重复完成
  		bCanRedoAfterFailure = true, --是否失败后可重复完成
  		bShowPrompt = false,	--不显示系统提示
  		bCanGiveUp = true, --是否能放弃
  		bFinishWhenDelvier = true,--接受即完成
  		bShowPrompt = true, --是否显示失败提示
		bHiddenTask = true, 		--隐藏任务
        bRecordFinishCount = true,				--是否记录完成次数
        iMaxFinishCount = 3,					--最大完成次数
        iClearFinishCountType = TaskClearFinishCountType.Daily,  --每天清空
		bUIButtonTask = true
	...
	AwardPreview =
	{
		...
		Items = function(TASKINTERFACE_POINTER)
			...
			local ItemId1 = 46576	--1到59级奖励礼包id
			local ItemId2 = 46617 --60到74奖励礼包id
			local ItemId3 = 46618 --75级以上奖励礼包 id
			...
			elseif PlayerLevel >= 60 and PlayerLevel < 75 then
				items = {
							Item1 = {ID = ItemId2, NUM = 1},--0时为？？？，其他显示物品和数量
			      }
			else
				items = {
							Item1 = {ID = ItemId3, NUM = 1},--0时为？？？，其他显示物品和数量
						}
			end
			return items
		end
	...
	,
	  ...
	  local IsCrossServer = TaskCApi.IsCrossServer (TASKINTERFACE_POINTER)
	    if IsCrossServer == true or nil then
   	  ...
   	  local iGuildlevel = TaskCApi.GetFactionLev(TASKINTERFACE_POINTER)   ---如果玩家在公会，返回公会等级，否则返回-1
	    if iGuildlevel < 3 then
		    return 11
        end

	  local iPlayerGong = TaskCApi.GetFactionContribution(TASKINTERFACE_POINTER)         ----- 取玩家公会贡献度
   	    if iPlayerGong < 100 then
		    return 34
		end

	  local iPlayerGX = TaskCApi.GetFactionCredit(TASKINTERFACE_POINTER)  --取玩家功勋值
      ...
      local iPlayerHYD = TaskCApi.GetTaskCompleteTimesFromFinishTimeTaskList(TASKINTERFACE_POINTER,13122) --获取活跃度任务完成次数
	    if TaskCApi.HasBuff(TASKINTERFACE_POINTER,2164) or iPlayerHYD >= 80 then --玩家是vip(bufid 2164)或者活跃度大于等于80
			return 0
	    else
			return 130
	    end
 ...
 --完成该任务时服务器调用
   	OnAward = function(TASKINTERFACE_POINTER, bSuccess,iParam,bRetrieve,ucRandomNum1,ucRandomNum2)
		if bSuccess then
			...
			if PlayerLevel < 60 then
				TaskCApi.DeliverRandomGift(TASKINTERFACE_POINTER,46576)    ----发随机礼包
